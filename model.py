...
class CSABlock(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        k = self.key(x).view(B, L, self.n_head, head_dim).transpose(1, 2)
        q = self.query(x).view(B, L, self.n_head, head_dim).transpose(1, 2)
        v = self.value(x).view(B, L, self.n_head, head_dim).transpose(1, 2)
        ### YOUR CODE HERE ###
